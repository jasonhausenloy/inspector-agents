# Host 3 handoff — Verifier Challenge overnight run

You're **host 3 of 3**. Hosts 1, 2, 3 all run the same 8-phase schedule on
...
"done criteria" needs ≥2 of 3 hosts surviving, so a partial run on host 3 is
still useful.
...
it from host 3** — host 1 will run it.
...
note which one is canonical for host 3 (use the one that's complete).
